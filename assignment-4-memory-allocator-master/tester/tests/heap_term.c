...
DEFINE_TEST(term) {
    ...
    assert(heap_init(0));
    assert(_malloc(REGION_MIN_SIZE - 2 * offsetof(struct block_header, contents) - BLOCK_MIN_CAPACITY - BLOCK_MIN_CAPACITY - 8 + (offsetof(struct block_header, contents) & 7)));
